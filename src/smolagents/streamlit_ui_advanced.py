...
from smolagents.streamlit_ui import StreamlitUI, stream_to_streamlit


class AdvancedStreamlitUI(StreamlitUI):
    """
    Advanced Streamlit interface for interacting with a [`MultiStepAgent`].

    This class extends `StreamlitUI` with advanced features including:
    - Model registry management and per-step model selection
    - Agent parameters configuration
    - Tool management (add/remove/import tools)
    - Executor configuration (for CodeAgent)
    - Prompt template editing
    - Function access panel
    - Settings persistence (save/load JSON)
    - Enhanced monitoring (token usage, memory inspector)

    For basic usage, use `StreamlitUI` instead.
    ...
    Example:
        ```python
        from smolagents import CodeAgent, AdvancedStreamlitUI, InferenceClientModel

        model = InferenceClientModel(model_id="meta-llama/Meta-Llama-3.1-8B-Instruct")
        agent = CodeAgent(tools=[], model=model)
        advanced_ui = AdvancedStreamlitUI(agent, file_upload_folder="uploads")
        advanced_ui.run()
        ```
    """
    ...
    def run(self):
        """Run the advanced Streamlit app with full settings and controls."""
        ...
        with st.sidebar:
            ...
            # Rebuild agent option
            if st.button("🔄 Rebuild Agent", use_container_width=True, help="Rebuild agent from original configuration. Use if agent is in a bad state."):
                st.session_state.agent_instance = self.agent
                st.session_state.model_registry = self.model_registry.copy()
                st.session_state.agent_instance.memory.reset()
                st.session_state.messages = []
                st.session_state.image_uploads = []
                st.session_state.file_uploads_log = []
                st.success("Agent rebuilt from original configuration!")
                st.rerun()
        ...
        if prompt := st.chat_input("Enter your prompt here"):
            ...
            # Process with agent (inside the prompt guard)
    ...
    def _render_model_config(self, st):
        ...
        # Option to update primary model
        st.markdown("**Primary Model**")
        primary_model_key = st.selectbox(
            "Primary Model (used as default)",
            options=registry_keys,
            index=0,
            key="primary_model_select",
            help="This model is used as the default for all steps unless overridden above",
        )
        
        if st.button("Apply Model Configuration", key="apply_model_config"):
            # Get models from registry
            primary_model = st.session_state.model_registry.get(primary_model_key, st.session_state.model_registry["Primary"])
            planning_model = st.session_state.model_registry.get(planning_model_key, primary_model)
            action_model = st.session_state.model_registry.get(action_model_key, primary_model)
            final_answer_model = st.session_state.model_registry.get(final_answer_model_key, primary_model)
            
            # Update agent's primary model
            agent.model = primary_model
            
            # Update per-step models
            agent.planning_model = planning_model
            agent.action_model = action_model
            agent.final_answer_model = final_answer_model
            
            # Update the "Primary" key in registry to reflect the new primary
            st.session_state.model_registry["Primary"] = primary_model
